...
client = OpenAI(api_key='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')
...
def translate_text_with_gpt3(text, target_lang):
    ...
    Returns:
        str: Translated text.
    """
    prompt = f"The following are the codes for languages - en is for English, te-IN is for Telugu, hi-IN is for Hindi, ta-IN is for Tamil, mr-IN is for Marathi, bn-IN is for Bengali, gu-IN is for Gujarathi\nTranslate the following text to {target_lang}: {text}"

    response = client.completions.create(model="text-davinci-002", prompt=prompt)

    return response.choices[0].text.strip()

# Replace the translate_text function in your original code
def translate_text(input, targetLang, sourceLang):
    """Translates from sourceLang to targetLang. If sourceLang is empty,
    it will be auto-detected.

    Args:
        input (str): Sentence to translate
        targetLang (str): i.e. "en"
        sourceLang (str, optional): i.e. "es" Defaults to None.

    Returns:
        str: Translated text
    """
    if sourceLang:
        # Use GPT-3 for translation
        translated_text = translate_text_with_gpt3(input, targetLang)
